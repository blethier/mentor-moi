  ...
  <div class="container  h-full  mt-6 mx-auto px-4">
    <div class="flex w-full my-8 justify-center items-end">
      <div v-if="isLoggedIn" class="relative mr-4 lg:w-full xl:w-1/2 w-2/4">
        <input
          type="search"
          placeholder="Filtrer les mentors par technologies"
          v-model="search"
          class=" bg-darkSands placeholder-white w-full bg-opacity-75 rounded border-4 border-darkSands focus:ring-2 focus:outline-none text-base outline-none darkMode py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
          data-kwimpalastatus="alive"
          data-kwimpalaid="1609853631472-5"
        />
      </div>
    </div>
    ...
    <p v-if="loading" class="text-center">
      <font-awesome-icon
        class="animate-spin text-darkSands text-5xl"
        icon="sync-alt"
      />
...
export default {
  ...
  mounted() {
    this.loadMentors(), this.loadOneMentor()
  },
  computed: {
    filteredMentors() {
      return this.filter()
